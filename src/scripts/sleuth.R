...
option_list <- list(
  ...
  make_option(c('-a', '--annotation'), type='character', default=character(0),
              help='Path to the annotations tsv.'),
  ...
  make_option(c('-s', '--sleuth'), type='character', default=character(0),
              help='Path to sleuth object to save.')
)
...
if (length(conditions) > 1) {
  ...
  for (i in 1:length(conditions)) {
    ...
    for (name in condition_name) {
      if (startsWith(name, 'control-') | i+1 > length(conditions)) {
        next
      }
      ...
      for (j in (i+1):length(conditions)) {
        ...
        for (name2 in condition2_name) {
          if (startsWith(name2, 'control-') | name == name2) {
            next
          }
...
so_file = paste(base_dir, 'sleuth_object.rds', sep='/')
