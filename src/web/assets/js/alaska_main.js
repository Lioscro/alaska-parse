...
function _saveAll(callback = function () {}) {
  _saveProject(function () {
    _saveSamples().then(function () {
      callback();
    });
  });
}
...
/**
 * Get all form inputs and save it to the database.
 */
function _getAllInputsAndSaveAll(callback = function () {}) {
  // Get all inputs and set objects.
  convert_all_meta_inputs();

  // Set progress to saved.
  project.set('progress', 'saved');

  _saveAll(callback);
}

/**
 * Refetch project from the server so that it is up to date.
 */
function _refetchProject(callback = function () {}) {
  // Update project.
...
/**
 * Call cloud function. Wrapper around Parse.Cloud.run
 */
function _runCloudFunction(name, callback = function (response) {}, args = {}) {
  Parse.Cloud.run(name, args).then(callback, function (error) {
    console.log(error);
    _showErrorModal(error);
  });
}
 ...
 */
function _setServerStatusBadge(badge, message) {
  badge.removeClass();
  ...
  if (message.toLowerCase() == 'online') {
    ...
    // remove popover
    var popover = $('#new_proj_btn_span');
    popover.popover('hide');
    popover.popover('disable');
    popover.popover('dispose');

  } else if (message.toLowerCase() == 'offline') {
    // Server is offline.
    badge.addClass('badge-danger');
  } else {
    // Any other message, display in yellow.
...
function _setServerStatus() {
  ...
  var versionBadge = $('#server_version_badge');
  ...
  Parse.Config.get().then(function (response) {
    ...
    _setServerStatusBadge(statusBadge, status);
...
/**
 * Parse URL parameters, including returning to a project.
 */
 ...
 * Resume project.
 */
function _resumeProject() {
  var progress = project.get('progress');

  if (progress == 'init') {
    // Project was initialized. Show ftp info.
    showFtpInfo();
  } else if (progress == 'meta') {
    // Samples were set. Show meta input.
    showMetaInput();
  } else if (progress == 'saved') {
    // Show meta input.
    showMetaInput(loadMetaInput);
  } else {
    // Otherwise, assume analysis is running.
    showProgress();
  }
}
...
function loadMetaInput() {
  var projectInputs = project.get('formInput')['project'];
  var commonInputs = project.get('formInput')['common'];

  // Set each of the forms.
  set_proj_meta_inputs(projectForm, projectInputs);
  ...
  for (var name in samples) {
    ...
    set_sample_meta_inputs(sampleForms[name], sample.get('formInput'));
  }
}

function _loadJobs(callback = function () {}) {
  project.relation('jobs').query().include('analysis').ascending('analysis.step').find().then(function (response) {
    console.log(response);
    ...
    for (var i = 0; i < response.length; i++) {
      var job = response[i];
      var code = job.get('analysis').get('code');
...
async function _loadReads(callback = function () {}) {
  reads = {};
  for (var name in samples) {
    var sample = samples[name];

    var rs = await sample.relation('reads').query().find();
    reads[name] = rs;
  }
...
function _loadSamples(callback = function () {}) {
  project.relation('samples').query().find().then(function (response) {
    console.log(response);

    samples = {};
    for (var i = 0; i < response.length; i++) {
      var sample = response[i];
      samples[sample.get('name')] = sample;
    }
 ...
 */
function _loadProject(objectId, callback = function () {}) {
  var query = new Parse.Query('Project');
  query.get(objectId).then(function (response) {
    project = response;
    callback();
  }, function (error) {
    console.log(error);
...
function _loadAll(objectId, callback = function () {}) {
  _loadProject(objectId, function () {
    _loadJobs(function () {
      _loadSamples(function () {
        _loadReads().then(function (response) {
          callback();
        }, function (error) {
          _showErrorModal(error);
        })
...
/**
 * Helper function to show the loading spinner inside a button.
 */
...
function _showSuccessCheck(button) {
  ...
  button.removeClass('running');
  button.width(width);
  button.children('.success_check').show();
}
...
function newProject() {
  ...
  if (project == null || project == undefined) {
    ...
    // Request new project from parse server.
    // This is a promise.
    _runCloudFunction('newProject', function (response) {
      console.log(response);

      // Check the returned object has an id.
      if ('id' in response) {
        // Show success check.
        _showSuccessCheck(btn);
...
function showFtpInfo() {
  // Make sure start button is disabled.
  $('#new_proj_btn').prop('disabled', true);

  var ftpDiv = $('#ftp_info_div');

  // First, set the project id and ftp info before showing.
  ftpDiv.find('#proj_id').text(project.id);
  Parse.Config.get().then(function (response) {
    var hostname = response.get('hostname');
    ftpDiv.find('#ftp_host').text(hostname);
  }, function (error) {
    console.log(error);
    _showErrorModal(error);
  });
  ftpDiv.find('#ftp_id').text(project.id);
  ftpDiv.find('#ftp_pw').text(project.get('ftpPassword'));

  // Add on click handler for fetch reads button.
  ftpDiv.find('#fetch_reads_btn').click(getReads);

  // Add on click handler for copy test reads.
  ftpDiv.find('#copy_test_reads_btn').click(copy_test_reads);

  ftpDiv.find('#refetch_reads_btn_2').click(regetReads);

  // Change url.
  // If we are testing, keep testing variable.
  if (testing) {
    history.pushState(null, '', '/?id=' + project.id + '&testing=true');
  } else {
    history.pushState(null, '', '/?id=' + project.id);
  }
  ftpDiv.find('#proj_url').text(window.location.href);

  // Set click handler for "View supported structures" button and
  // "View FTP info" button.
  ftpDiv.find('#read_example_btn').click(function () {
  ...
  ftpDiv.find('#ftp_info_btn').click(function () {
  ...
  scrollToElement(ftpDiv);
}

function _getReadsFail() {
...
function _getReadsSuccees() {
  ...
  scrollToElement(succeeded);
}

/**
 * Set the raw reads table with the given reads.
 */
function _setReadsTable() {
  ...
  for (var i = 0; i < keys.length; i++) {
    var read = reads[keys[i]];
    var folder = keys[i];
    for (var j = 0; j < read.length; j++) {
      // Get values from reads array.
      var path = read[j].path;
      var filename = path.replace(/^.*[\\\/]/, '');
      var size = read[j].size;
      ...
      // Calculate md5 sum.
      _getMd5(path, row.children('#' + new_md5_id), read[j]);
    }
  }
}

function _getMd5(path, td, read) {
  _runCloudFunction('getMd5', function (response) {
    console.log(response);

    read.md5Checksum = response;
...
function getReads() {
  ...
  // Call cloud function getReads
  _runCloudFunction('getReads', function (response) {
    console.log(response);

    // Check how many reads were found.
    if (Object.keys(response).length > 0) {
      reads = response;
      _setReadsTable(response);
...
/**
 * Set sample names in samples names modal.
 */
function _setNamesModal() {
  var modal = $('#sample_names_modal');

  var sampleNames = Object.keys(reads).sort();
  var header = modal.find('#sample_names_header');
  header.text(header.text().replace('num', sampleNames.length));
  ...
  for (var i = 0; i < sampleNames.length; i++) {
    ...
    var new_folder_id = folder_id.replace('SAMPLEID', name);
    var new_name_id = name_id.replace('SAMPLEID', name);

    var row = $('#' + row_id).clone();

    // Set row id.
    row.attr('id', row_id.replace('SAMPLEID', name));
    ...
    // Then, set the values.
    row.children('#' + new_folder_id).text(name);

    var textbox = row.children('#' + new_name_id);
    textbox.attr('placeholder', 'Default: ' + name);
...
/**
 * Helper function to bind buttons for raw reads.
 */
function bindReads() {
  // Add on click handler for refetch reads button.
  $('#refetch_reads_btn_1').click(regetReads);

  // Bind done button for inputing sample names.
  $('#sample_names_btn').click(_getNamesAndSetSamples);
}

function _getNamesAndSetSamples() {
  var btn = $(this);
  _showLoadingSpinner(btn);
  ...
  for (var name in reads) {
    ...
    if (val != '' && val != null) {
      if (!(val in samples) && !(val.includes('\\')) && !(val.includes('/')) && !(val.includes('$')) && !(val.includes('%')) && !(val.includes('~'))) {
        names[val] = input;
      } else {
        input.addClass('is-invalid');
  ...
  if (valid) {
    ...
    for (var name in reads) {
      ...
      // If the input is empty, just use default (i.e. do nothing).
      if (val != '') {
        samples[val] = reads[name];
      } else {
        samples[name] = reads[name];
      }
    }
    ...
    }, {objectId: project.id, samples: samples});
  }
}

function _setupSamplesInput() {
  var sampleForm = $('.sample_collapse');
  var sampleNames = Object.keys(samples).sort();

  // Add samples in order by name.
  sampleForms = {};
  for (var i = 0; i < sampleNames.length; i++) {
    var name = sampleNames[i];
    var sample = samples[name];
    var newSampleForm = sampleForm.clone(true);

    // Replace all instances of SAMPLEID to the id.
    var html = newSampleForm.html();
    newSampleForm.html(html.replace(new RegExp('SAMPLEID', 'g'), name));

    sampleForms[name] = newSampleForm;

    // Set the sample name.
    newSampleForm.find('.sample_name_group').find('input').val(name);

    // Set the reads table for this sample.
    _setupReadsTable(sample, newSampleForm);

    // Characteristics.
    var charGroup = newSampleForm.find('.sample_specific_'
                                       + 'characteristics_group');
    set_fluid_input_rows(charGroup.find('.sample_characteristics_inputs'));

    // Save changes button.
    var saveChangesBtn = newSampleForm.find('.save_btn');
    saveChangesBtn.click(function () {
        var btn = $(this);
        btn.prop('disabled', true);

        _getAllInputsAndSaveAll(function () {
          show_saved(btn, function() {});
        });
    });

    // Set up copy buttons.
    var copyBtns = newSampleForm.find('.copy_btn');
    copyBtns.each(function () {
      _setupCopyBtn($(this), name);
    })

    // Append new form.
    $('#sample_card').append(newSampleForm);
  }

  // Then, set the button handler.
  var dropdown = $('#sample_choices');
  _setupSampleChoices(dropdown, sampleForms);
}

function _setupSampleChoices(dropdown, forms) {
  ...
  for (var name in forms) {
    ...
    new_item.click({'name': name}, function (e) {
      ...
      grandparent.children('button').text(name);
      _showSampleForm(name, sampleForms[name]);
    });
...
function _showSampleForm(name, form) {
  // If the form is already being shown, just return.
  if (currentSampleForm == form) {
    return;
  }

  // If the global currentSampleForm is not empty,
  // then we need to hide it first.
  if (currentSampleForm != null) {
    // Set on-hide handler to show the new sample form.
    currentSampleForm.on('hidden.bs.collapse', function () {
      $(this).off('hidden.bs.collapse');
      form.collapse('show');
    });
    ...
    currentSampleForm.collapse('hide');
  }
  else {
    form.collapse('show');
  }
  ...
  currentSampleForm = form;
}

function _setupCopyBtn(btn, nameToIgnore) {
  // Set up the popover.
  ...
  btn.popover({
    ...
    'content': function () {
      ...
      var input = $('<input type="checkbox" name="copy">');
      ...
      for (var i = 0; i < sampleNames.length; i++) {
        ...
        if (name == nameToIgnore) {
          label_copy.addClass('disabled');
        }
      }
...
function _setupReadsTable(sample, form) {
  // Set the reads table.
  ...
  sample.relation('reads').query().find().then(function (response) {
    ...
    for (var i = 0; i < response.length; i++) {
      ...
      var md5 = read.get('md5Checksum');
      var size = read.get('size');
...
function _setupCommonInput() {
  commonForm = $('#sample_common_form');
  var form = commonForm.children('form');

  // Set shared inputs.
  _setupSharedInputs(form);
  ...
  commonForm.find('.save_btn').click(function () {
    var btn = $(this);
    ...
    function show_next_form() {
      ...
      meta.show();
      $('#meta_footer').show();
      scrollToElement(meta);
    }

    _getAllInputsAndSaveAll(function () {
      show_saved(btn, show_next_form);
    });
  });
}

function _setupProjectInput() {
  projectForm = $('#proj');

  // Set the project id header
  var html = projectForm.html();
  projectForm.html(html.replace(new RegExp('PROJECT_ID', 'g'), project.id));

  // Set up contributors.
  var contributors_group = projectForm.find('.proj_contributors_group');
  ...
  var factor_card = projectForm.find('.factor_card');
  set_factor(factor_card);

  // Enable experimental design.
  var design_group = projectForm.find('.proj_experimental_design_group');
  ...
  projectForm.find('.save_btn').click(function () {
    var btn = $(this);
  ...
  if (Object.keys(samples).length < 8) {
    factor_show_radio.prop('disabled', true);
  }
}

/**
 * Setup samples, common, and project metadata input forms.
 */
function _setupMetaInput(callback = function () {}) {
  _setupSamplesInput();
  _setupCommonInput();
  _setupProjectInput();
...
function showMetaInput(callback = function () {}) {
  ...
  _runCloudFunction('getOrganismsDict', function (response) {
    ...
    // Setup meta input first.
    _setupMetaInput(function () {
 ...
 */
function _setupSharedInputs(form) {
  ...
  _setupOrganisms(organism_inputs);
...
function _setupOrganisms(inputs) {
...
function showProgress() {
  ...
  progress_container.find('#retry_btn').click(function () {
    $(this).hide();
    startProject(function (response) {
      // Response contains job info.
      jobs = response.sort(function (a, b) {
        a.get('analysis').get('step') - b.get('analysis').get('step');
      });
...
function get_output(type, textarea, ul) {
  _runCloudFunction('getOutput', function (response) {
    // console.log(response);
    textarea.val(response.join(''));
    ...
    for (var i = 0; i < response.length; i++) {
      ...
      if (line.startsWith('##')) {
        ...
        ul.append($('<li>' + line.substring(3, line.length) + '</li>'));
      }
    }
...
function set_progress() {
  var status = project.get('oldProgress');
  var progress_container = $('#progress_container');
 ...
 */
function set_organisms_inputs_listeners(genus_select, species_select, version_select) {
  ...
  species_select.change({
    ...
    if (genus != null && genus != '' && genus in organisms
      ...
      // Then, add the correct elements.
      var versions = Object.keys(organisms[genus][species]).sort();
...
function show_sample_form(id, form) {
  ...
  if (currentSampleForm != null) {
    ...
    currentSampleForm.on('hidden.bs.collapse', function () {
      form.collapse('show');
      currentSampleForm.off('hidden.bs.collapse');
    });
...
function set_values_of_organism_dropdowns(div, vals) {
  var inputs = div.find('.sample_organism_inputs');
  var selects = inputs.children('select');
  var genus_select = selects.eq(0);
  var species_select = selects.eq(1);
  var version_select = selects.eq(2);
  ...
  var speciesHandler = function () {
    ...
    species_select.change(versionHandler);
    species_select.change();
  };

  genus_select.change(speciesHandler);
  genus_select.change();

  setTimeout(function() {
    species_select.unbind('change', versionHandler);
    genus_select.unbind('change', speciesHandler);
  }, 500)
}
...
function set_factor_card_to_sample_listener(factor_card,
  ...
  name_inputs.change({
    ...
    for (var name in sampleForms) {
      var sample_form = sampleForms[name];
  ...
  name_div.find('select').change({
    ...
    for (var factor_name in factor_names_to_class_names) {
      ...
      var form_group = commonForm.find('.' + class_name);
...
function set_proj_meta_input() {
  ...
  // Set the project id header
  var html = projectForm.html();
  projectForm.html(html.replace(new RegExp('PROJECT_ID', 'g'), proj_id));
  ...
  enable_popovers_tooltips(projectForm);
}
  ...
  for (var name in sampleForms) {
    ...
    copy.find('*').each(function () {
      ...
      if (ele_name != null && ele_name != '' && ele_name.includes('_share_')) {
        ele.attr('name', ele_name.replace('_share_', '_' + name + '_'));
      }

      if (ele_id != null && ele_id != '' && ele_id.includes('_share_')) {
        ele.attr('id', ele_id.replace('_share_', '_' + name + '_'));
      }

      if (ele_for != null && ele_for != '' && ele_for.includes('_share_')) {
        ele.attr('for', ele_for.replace('_share_', '_' + name + '_'));
      }
    });
    ...
    if (class_name == 'sample_read_type_group') {
      ...
      var type = inputs.find('input:radio:checked').val();
      if (type == 'single') {
        single_collapse.collapse('show');
        paired_collapse.collapse('hide');
      } else {
...
function refresh_checkbox(checkbox) {
  ...
  if (checkbox.prop('disabled')) {
    ...
    if (class_name == 'sample_read_type_group'
        && form_group.find('input:radio:checked').val() == 'paired') {
...
function set_common_meta_input(cb) {
  ...
  // Enable popovers and tooltips.
  enable_popovers_tooltips(commonForm);
}
...
function get_values_from_read_type(form_group) {
  var inputs_div = form_group.children('div:last');
  var read_type = inputs_div.find('input:radio:checked').val();
...
function get_proj_meta_inputs(card) {
  ...
  var old = project.get('formInput');
  old['project'] = inputs;
  project.set('formInput', old);
...
function set_common_meta_inputs(card, inputs) {
  for (var class_name in common_meta_classes_to_functions) {
    var form_group = card.find('.' + class_name).eq(0);
    var checkbox = form_group.find('input:checkbox');
  ...
  if ('sample_read_type_group' in inputs) {
    if (inputs['sample_read_type_group'].type == 'single') {
...
function get_sample_meta_inputs(card) {
  ...
  var sample = samples[inputs['sample_name_group']];
  sample.set('formInput', inputs);

  return inputs;
}

function validate_proj_meta_inputs(inputs, projectForm) {
  var valid = true;
  var failed_fields = {};

  for (var class_name in proj_meta_classes_to_functions) {
    var val = inputs[class_name];
    var form_group = projectForm.find('.' + class_name);
...
function validate_common_meta_inputs(inputs, commonForm) {
  var valid = true;
  var failed_fields = {};
...
function validate_all_meta_inputs(modal) {
  ...
  var proj_inputs = get_proj_meta_inputs(projectForm);
  var failed_inputs = validate_proj_meta_inputs(proj_inputs, projectForm);
  ...
  // Common inputs.
  var common_inputs = get_common_meta_inputs(commonForm);
  failed_inputs = validate_common_meta_inputs(common_inputs, commonForm);
  ...
  for (var name in sampleForms) {
    ...
    if (Object.keys(failed_inputs).length > 0) {
      ...
      title_text = 'Sample Metadata Form: ' + name;
...
function fill_with_test_metadata() {
  set_proj_meta_inputs(projectForm, test_proj_inputs);
  set_common_meta_inputs(commonForm, test_common_inputs);
...
function set_all_meta_inputs() {
  ...
  read_object_from_temp('proj_inputs', function (obj, form) {
    ...
    read_object_from_temp('common_inputs', function (obj, form) {
      ...
      for (var name in sampleForms) {
        var sample_form = sampleForms[name];
        read_object_from_temp('sample_' + name + '_inputs', function (obj, form) {
...
function convert_proj_meta_inputs(card) {
  ...
  for (var class_name in inputs) {
    ...
    switch (class_name) {
      case 'proj_corresponding_group':
        obj['contributors'].unshift(input)
        break;
      case 'proj_corresponding_email_group':
        obj['email'] = input;
        break;
      ...
      case 'proj_title_group':
        obj.metadata['title'] = input;
        break;

      case 'proj_abstract_group':
        obj.metadata['abstract'] = input;
        break;

      case 'proj_sra_center_code_group':
        obj.metadata['sraCenterCode'] = input;
    }
  }
...
function convert_sample_meta_inputs(card) {
  ...
  for (var class_name in inputs) {
    ...
    switch (class_name) {
      ...
      case 'sample_characteristics_group':
        for (var i = 0; i < input.length; i++) {
          obj.metadata[input[i][0]] = input[i][1];
        }
        break;

      case 'sample_description_group':
        obj.metadata['description'] = input;
        break;
      ...
      case 'sample_genotype_group':
        obj.metadata['genotype'] = input;
        break;

      case 'sample_growth_conditions_group':
        obj.metadata['growth conditions'] = input;
        break;

      case 'sample_library_preparation_group':
        obj.metadata['library preparation'] = input;
        break;
      ...
      case 'sample_organism_group':
        if (input.genus && input.species && input.version) {
          obj['reference'] = {'__type': 'Pointer',
                             'className': 'Reference',
                             'objectId': organisms[input.genus][input.species][input.version]};
        }
        break;

      case 'sample_organism_strain_group':
        obj.metadata['organism strain'] = input;
        break;

      case 'sample_read_type_group':
        obj['readType'] = input.type;

        if (input.type  == 'single') {
          obj['readLength'] = input.length;
          obj['readStd'] = input.stdev;
        } else if (input.type == 'paired') {
          obj['readPairs'] = input.pairs;
        }
        break;

      case 'sample_rna_extraction_group':
        obj.metadata['rna extraction'] = input;
        break;

      case 'sample_sequencing_platform_group':
        obj.metadata['platform'] = input;
        break;

      case 'sample_sequenced_molecules_group':
        obj.metadata['sequenced molecules'] = input;
        break;

      case 'sample_tissue_group':
        obj.metadata['tissue'] = input;
        break;
    }
  }
...
function convert_all_meta_inputs() {
  var proj_inputs = convert_proj_meta_inputs(projectForm);
  project.set(proj_inputs);

  var commonInputs = get_common_meta_inputs(commonForm);
  var inputs = project.get('formInput');
  inputs['common'] = commonInputs;
  project.set('formInput', inputs);

  for (var name in sampleForms) {
    var sample_form = sampleForms[name];
    var sample_inputs = convert_sample_meta_inputs(sample_form);

    samples[name].set(sample_inputs);
  }
}
...
function copy_input() {
  ...
  var input_group = currentSampleForm.find('.' + group);
  ...
  active_labels.each(function () {
    ...
    names_to_copy.push(checkbox.val());
  });
    ...
    for (var i = 0; i < control_groups.length; i++) {
      ...
      if (value == null || value == '') {
        ...
        for (var sampleName in samples) {
          ...
          if (sample.get('metadata')[name] == value) {
            chosen.push(sample);
          }
        }
        var sample_names = chosen.map(function (sample) {
...
function show_verify_meta_modal() {
  ...
  if (valid) {
    ...
    set_choose_controls_modal(new_modal, project.get('factors'));
